...
- **Tests**: `pytest` from repo root. Full suite is 1004 passed + 2 skipped + 1 pre-existing failure (`test_router.py::test_validate_all_models_passes_on_shipping_router`, unrelated to sprint work). New work must not regress the green count. When you add tests, bump this number in the same commit so the baseline stays honest.
