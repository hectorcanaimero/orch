...
The Python goldens and `cases.json` are frozen (see below). The Go goldens
still regenerate from the current renderer:

```bash
go test ./internal/prompt -run TestRenderMatchesTheGoGoldens -update
```

Read the diff. A golden nobody looked at pins whatever was there,
...
implementation — `make-goldens.py`, removed from `main` together with the Python tree
