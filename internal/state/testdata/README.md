...
```bash
# A venv with orch v0.11.0 (the python-legacy branch / the v0.11.0-py tag):
python3 -m venv /tmp/orch-py && /tmp/orch-py/bin/pip install -q -e .
/tmp/orch-py/bin/python internal/state/testdata/make-fixture.py \
    internal/state/testdata/orch-py-0.11.0.db
```
...
The generator. Kept in the repo so the fixture is reproducible and so the
rows it writes are reviewable without opening a binary file.

It imports `orchestrator.*` — the Python tree. It is **not** run by `go test`,
and it will stop working once the Python tree is deleted. At that point the
`.db` stays as a frozen artifact of the format we promised to keep reading,
which is fine: by then nothing can regenerate it, and nothing should.
