...
// "you asked for something that does not exist" from "not yet".
var ErrNotPorted = errors.New("backend not ported to Go yet")
...
func Get(name model.Backend) (Provider, error) {
	...
	return nil, fmt.Errorf("unknown backend %q; expected one of [claude codex]", name)
}
