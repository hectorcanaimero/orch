...
// TestPromptDelivery pins how each ported CLI wants its prompt. claude takes
// it on stdin; the engine reads this rather than assuming, because gemini and
// agy will answer PromptArg when they land.
func TestPromptDelivery(t *testing.T) {
	for _, p := range []Provider{ClaudeProvider{}} {
