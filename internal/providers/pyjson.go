...
// `_iter_jsonl_events` does: a SIGKILLed codex leaves a half-written final
// line, and that must not cost us the events before it.
//
...
// pyFloat renders v exactly as Python's repr(float) does, because claude's
// `--max-budget-usd` value is built by dispatcher.py as `str(budget)` and the
// flag should read the same from either binary.
//
// This is a second copy of internal/state's pyFloat, which carries the full
// explanation of CPython's rule and a golden table. The duplication is
// deliberate for now: checklist rule 14 keeps this package off everything but
// internal/model, so the shared home would have to be internal/model itself —
// a move that touches another lane's package mid-migration. Flagged in
// docs/brainstorm/go-migration-notes.md for a follow-up.
...
// positive, finite v, plus the base-10 exponent of its leading digit.
