...
var versionDriftMarkers = []string{
	...
	"unsupported model",
	// claude 2.1.269 rejects an unknown --model with exactly this sentence
	// and a `[claude-code:unrecognized_model]` stderr line mixed into stdout.
	// None of the markers above appear in it, so the fallback_cli_model retry
	// never fired for the one case it exists for. Fixture:
...
var permissionMarkers = []string{
	...
	// Real auth failures from the CLIs use these spellings, which are
	// substrings of neither of the two above: claude sends
	// `code: "authentication_error"`, codex says "auth expired".
	"authentication error",
	"authentication_error",
	"auth expired",
}

var permissionCodes = []string{"401", "403"}
...
var transientCodes = []string{"500", "502", "503", "504"}
...
func Classify(res Result) Failure {
	...
	case containsAny(blob, rateLimitMarkers), hasStatusCode(blob, rateLimitCodes):
		return FailureRateLimit
	// 4) PERMISSION — auth errors are terminal; no retry.
	case containsAny(blob, permissionMarkers), hasStatusCode(blob, permissionCodes):
	...
	case containsAny(blob, transientMarkers), hasStatusCode(blob, transientCodes), errCodeRe.MatchString(blob):
...
// hasStatusCode reports whether any of codes appears in haystack as a
// standalone number.
//
// The HTTP status codes used to be matched as bare substrings, and the
// haystack includes 2 KB of stdout — for claude, a JSON envelope that is
// nothing but numbers. `"cache_read_input_tokens":40321` contains "403", so a
// truncated envelope (a PARSER failure, retryable) classified as PERMISSION
// (terminal, never retried). A code only counts when it is glued to neither a
// digit nor a decimal point on either side.
//
// Python writes this as `(?<![0-9.])<code>(?![0-9.])`. Go's RE2 has no
// lookbehind, so the same rule is applied by hand — which also avoids the
// trap in the obvious regexp rewrite, `(^|[^0-9.])code([^0-9.]|$)`: that
// consumes the separator, so the second code in "429 429" would be missed.
func hasStatusCode(haystack string, codes []string) bool {
	for _, code := range codes {
		for from := 0; from+len(code) <= len(haystack); {
			i := strings.Index(haystack[from:], code)
			if i < 0 {
				break
			}
			at := from + i
			if !isDigitOrDot(haystack, at-1) && !isDigitOrDot(haystack, at+len(code)) {
				return true
			}
			// Advance by one, not by len(code): overlapping spellings such
			// as "50500" must still be able to match at the later offset.
			from = at + 1
		}
	}
	return false
...
// isDigitOrDot reports whether s[i] is a digit or a decimal point. An index
// outside s is neither — start and end of string are both boundaries.
func isDigitOrDot(s string, i int) bool {
	if i < 0 || i >= len(s) {
		return false
	}
	c := s[i]
