...
- **G1.6 — landed**: `validate`, `graph`.
- **G4.6 — landed**: `migrate` (narrower than Python — see its table row).
- **G1.6 / G2+ — not yet in this table**: `router`, `config`, `atomize`,
  `stop`, and everything else in Python's `_SUBCOMMANDS`. Add a row here in
  the same PR that lands one.
