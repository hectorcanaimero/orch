...
- [ ] 5.3 Create `frontend/src/hooks/useTunnelCapabilities.ts`: React Query one-shot (`staleTime: Infinity`, `retry: false`) — drives the conditional mount from `BoardPage`. **Cite**: S:TUN-4, D#9. **Done**: hook exports `useTunnelCapabilities()`.
...
- [ ] 5.7 Modify `frontend/src/pages/BoardPage.tsx`: import `TunnelPanel` and render it ONLY when `useTunnelCapabilities().data?.can_control === true` — no CSS-hidden fallback (D#9). When `enabled:false` or gates fail, do NOT mount the panel at all. **Cite**: S:TUN-NFR-3, D#9. **Done**: panel visible for operator on loopback; absent for stakeholder or tunnel-origin caller.
...
`1.1 (procutil) → 1.3 (arch refactor) → 2.7 (manager uses procutil) → 2.8 (manager tests) → 3.1 (server wires manager) → 3.4 (/start route) → 3.7 (endpoint tests) → 4.3 (auto_start uses /start path) → 4.4 (auto_start tests) → 5.6 (TunnelPanel consumes /start) → 5.7 (BoardPage mounts panel) → 6.3 (final smoke).`
